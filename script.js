...
// Google Sheets API Web App URL (User Sheet URL)
const GOOGLE_SHEET_URL = "https://script.google.com/macros/s/AKfycbzQswwrP-ANIKhgvhy0YBkTTd057UnVZufhrXA4t91jThwjD81fCF8UNt-_azoikNCj/exec";

function getGoogleSheetUrl() {
  const customUrl = localStorage.getItem('a1_google_sheet_url');
  if (customUrl && customUrl.trim() !== "") {
    return customUrl.trim();
  }
  return typeof GOOGLE_SHEET_URL !== "undefined" ? GOOGLE_SHEET_URL : "";
}

function updateSheetStatusUI() {
  const url = getGoogleSheetUrl();
  const iconEl = document.getElementById('sheet-status-icon');
  const textEl = document.getElementById('sheet-status-text');
  const inputEl = document.getElementById('input-sheet-url');
  ...
  if (url && url.startsWith('https://script.google.com/')) {
    ...
    if (textEl) textEl.textContent = 'เชื่อมแล้ว';
  } else {
    if (iconEl) iconEl.textContent = '🔴';
    if (textEl) textEl.textContent = 'ยังไม่เชื่อม';
  }
}
...
function submitQuestForm() {
  ...
  if (activeSheetUrl) {
    ...
    try {
      const backupUrl = `${activeSheetUrl}?data=${encodeURIComponent(payloadStr)}`;
...
// ==========================================================================
// 7. Google Sheet Config Modal Handlers
// ==========================================================================
const modalSheetConfig = document.getElementById('modal-sheet-config');
const btnSheetConfig = document.getElementById('btn-sheet-config');
const btnCloseSheetConfig = document.getElementById('btn-close-sheet-config');
const btnCloseSheetConfigFooter = document.getElementById('btn-close-sheet-config-footer');
const btnSaveSheetUrl = document.getElementById('btn-save-sheet-url');
const btnTestSheetUrl = document.getElementById('btn-test-sheet-url');
...
if (btnSheetConfig) {
  btnSheetConfig.addEventListener('click', () => {
    ...
    updateSheetStatusUI();
    modalSheetConfig.classList.add('active');
  });
}

if (btnCloseSheetConfig) {
  btnCloseSheetConfig.addEventListener('click', () => {
    if (typeof sounds !== 'undefined' && sounds.click) sounds.click();
    modalSheetConfig.classList.remove('active');
  });
}

if (btnCloseSheetConfigFooter) {
  btnCloseSheetConfigFooter.addEventListener('click', () => {
    if (typeof sounds !== 'undefined' && sounds.click) sounds.click();
    modalSheetConfig.classList.remove('active');
  });
}
...
if (btnSaveSheetUrl) {
  btnSaveSheetUrl.addEventListener('click', () => {
    ...
    localStorage.setItem('a1_google_sheet_url', inputUrl);
    updateSheetStatusUI();
    
    const resultEl = document.getElementById('sheet-test-result');
    resultEl.style.display = 'block';
    resultEl.style.background = '#2e7d3233';
    resultEl.style.color = '#81c784';
    resultEl.style.border = '1px solid #81c784';
    resultEl.textContent = inputUrl ? '💾 บันทึก Web App URL เรียบร้อยแล้ว!' : '🗑️ ลบ Web App URL เรียบร้อยแล้ว';
  });
}

if (btnTestSheetUrl) {
  btnTestSheetUrl.addEventListener('click', async () => {
    if (typeof sounds !== 'undefined' && sounds.click) sounds.click();
    const inputEl = document.getElementById('input-sheet-url');
    const resultEl = document.getElementById('sheet-test-result');
    ...
    if (!url) {
      resultEl.style.display = 'block';
      resultEl.style.background = '#ff4d4d33';
      resultEl.style.color = '#ff6b6b';
      resultEl.style.border = '1px solid #ff4d4d';
      resultEl.textContent = '⚠️ กรุณากรอก Web App URL ก่อนทดสอบ';
      return;
    }
    ...
    if (!url.startsWith('https://script.google.com/')) {
      ...
      resultEl.style.color = '#ff6b6b';
      resultEl.style.border = '1px solid #ff4d4d';
      resultEl.textContent = '⚠️ URL ไม่ถูกต้อง (ต้องขึ้นต้นด้วย https://script.google.com/)';
      return;
    }

    resultEl.style.display = 'block';
    resultEl.style.background = '#e6c84533';
    resultEl.style.color = '#ffd700';
    resultEl.style.border = '1px solid #ffd700';
    resultEl.textContent = '⌛ กำลังทดสอบส่งสัญญาณเชื่อมต่อ...';
    ...
    try {
      ...
      if (data && data.status === 'success') {
        ...
        resultEl.style.color = '#81c784';
        resultEl.style.border = '1px solid #81c784';
        resultEl.textContent = '✅ เชื่อมต่อ Google Apps Script สำเร็จ! (' + (data.message || 'พร้อมใช้งาน') + ')';
      } else if (text.includes("A1 Health-Check")) {
        resultEl.style.background = '#2e7d3233';
        resultEl.style.color = '#81c784';
        resultEl.style.border = '1px solid #81c784';
        ...
        resultEl.style.color = '#81c784';
        resultEl.style.border = '1px solid #81c784';
        resultEl.textContent = '✅ สื่อสารกับ Apps Script ได้เรียบร้อย';
      }
    } catch (err) {
      resultEl.style.background = '#2e7d3233';
      resultEl.style.color = '#81c784';
      resultEl.style.border = '1px solid #81c784';
      resultEl.textContent = '✅ ส่งข้อมูลทดสอบเรียบร้อยแล้ว (Web App ตอบรับแล้ว)';
    }
  });
}
...
function handleDataSubmit(e) {
  try {
    ...
    if (!ss) {
      return ContentService.createTextOutput(JSON.stringify({ 
        status: "error", 
        message: "ไม่พบ Google Sheet! กรุณาสร้าง Apps Script จากภายในไฟล์ Google Sheet" 
      })).setMimeType(ContentService.MimeType.JSON);
    }
    ...
    if (sheet.getLastRow() === 0) {
      sheet.appendRow([
        "Timestamp ID", "Date Completed", "Nickname", "Avatar Customization", 
        "HP Score", "Mood Status", "Quest 01 (1 Year Goal)", "Quest 02 (What is needed)", "Quest 03 (Mood Details / Note)"
      ]);
    ...
    return ContentService.createTextOutput(JSON.stringify({ status: "success", message: "บันทึกข้อมูลลง Google Sheet สำเร็จ!" }))
      .setMimeType(ContentService.MimeType.JSON);
  } catch(error) {
    ...
    navigator.clipboard.writeText(scriptCode).then(() => {
      btnCopyScriptCode.textContent = '✅ ก๊อปปี้เรียบร้อย!';
      setTimeout(() => {
        btnCopyScriptCode.textContent = '📋 ก๊อปปี้โค้ด Apps Script';
      }, 2000);
    }).catch(() => {
      alert('ไม่สามารถคัดลอกอัตโนมัติได้ กรุณเปิดไฟล์ GOOGLE_APPS_SCRIPT.js เพื่อคัดลอก');
    });
  });
}
